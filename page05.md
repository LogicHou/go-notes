...
Go 通过结构体类型 struct 提供聚合抽象能力

### 使用关键字 type 定义一个新类型（最常用）
...
如果一个新类型是基于某个 Go 原生类型定义的，那么这个原生类型为新类型的**底层类型**，被用来判断两个类型本质上是否相同

T1 和 T2 是不同类型，但底层类型都是类型 int，所以本质上是相同的

而本质上相同的两个类型，它们的变量可以通过**显式转型进行相互赋值**
...
可以拥有**自身类型的指针类型**、以自身类型为元素类型的切片类型，以及以自身类型作为 value 类型的 map 类型的字段：
...
但如果是指针、切片、map等类型，其本质都是一个int大小(指针，4字节或者8字节，与操作系统有关)，因此该结构体的大小是固定的
...
结构体类型通常是对真实世界复杂事物的抽象，**通常都要被赋予适当的初始值后，才会有合理的意义**

结构体类型变量的初始化大致分为了三种情况

#### 1. 零值初始化

当构成结构体类型字段各个变量的值都是零值时，这个结构体类型变量就处于零值状态：

    var book Book // book为零值结构体变量

这样定义的一本书既没有书名，也没有作者、页数、索引等信息，对这本书的抽象就失去了实际价值，对于像这样的结构体类型，使用零值初始化并不是正确的选择

但是采用零值初始化的零值结构体变量也有其存在的意义，如果一种类型采用零值初始化得到的零值变量，是有意义的，而且是直接可用的，这种类型可以称之为“零值可用”类型

定义零值可用类型是简化代码、改善开发者使用体验的一种重要的手段

比如标准库 sync 包的 Mutex 类型：

    var mu sync.Mutex // 不需要额外的显式初始化操作
    mu.Lock()
    mu.Unlock()
...
* 当结构体类型定义中的字段顺序发生变化，或者字段出现增删操作时，需要手动调整该结构体类型变量的显式初始化代码，让赋值顺序与调整后的字段顺序一致
...
Go 推荐使用 “field:value” 形式的复合字面值进行显式初始化：
...
“field:value”形式字面值中的字段可以以任意次序出现，未显式出现在字面值中的结构体字段将采用其对应类型的零值

采用类型零值时的符合字面值初始化：

    t := T{}

使用预定义函数 new 创建结构体变量实例（比较少用）：

    tp := new(T)

#### 使用特定的构造函数

如果一个结构体类型中包含未导出字段，并且这个字段的零值还不可用，或者一个结构体类型中的某些字段，需要一个复杂的初始化逻辑

这时可以使用**特定的构造函数**创建并初始化结构体变量
...
这个 runtimeTimer 结构体中的一些字段不是零值可用的，所以标准库提供了一个 Timer 结构体专用的构造函数 NewTimer：
    ...
    // $GOROOT/src/time/sleep.go
    func NewTimer(d Duration) *Timer { // 参数列表中的参数通常与 T 定义中的导出字段相对应
        c := make(chan Time, 1)
        t := &Timer{                   // 这里 t 是一个指针类型变量
            C: c,
            r: runtimeTimer{           // T 的非导出字段在 NewT 内部进行初始化
                when: when(d),         // 一些需要复杂初始化逻辑的字段也会在 NewT 内部完成初始化
                f:    sendTime,
                arg:  c,
            },
        }
        startTimer(&t.r)
        return t                       // 返回值则是一个 T 指针类型的变量
    }

    // 最后只要调用 NewT(NewTimer) 函数就可以得到一个可用的 T 指针类型变量了

### 结构体类型的内存布局

Go 结构体类型是既数组类型之后，第二个将元素（结构体字段）一个接着一个以“平铺”形式，存放在一个连续内存块中
...
对于结构体而言，它的变量的内存地址，只要是它最长字段长度与系统对齐系数两者之间较小的那个的整数倍就可以
