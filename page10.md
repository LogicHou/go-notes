...
将这些 Goroutine 按照一定算法放到“CPU”（这里指 Goroutine 调度器）上执行的程序，就被称为 Goroutine 调度器（Goroutine Scheduler）
...
Goroutine 调度器的任务是：**将 Goroutine 按照一定算法放到不同的操作系统线程中去执行**
...
* M(machine)    ：代表着真正的执行计算资源。在绑定有效的 P 后，进入一个调度循环，而调度循环的机制大致是**从 P 的本地运行队列以及全局队列中获取 G，切换到 G 的执行栈上并执行 G 的函数，调用 goexit 做清理工作并回到 M，如此反复**。M 并不保留 G 状态，这是 G 可以跨 M 调度的基础

Go 调度器从最初的 G-M 模型、到 G-P-M 模型，从不支持抢占，到支持协作式抢占，再到支持基于信号的异步抢占

### G-M 调度模型（已经废弃）

这种调度器的工作就是将 G 调度到 M 上去运行
...
常规情况下，如果某个 G 没有进行系统调用（syscall）、没有进行 I/O 操作、没有阻塞在一个 channel 操作上，**G 是被抢占调度的从而让 G 停下来并调度下一个可运行的 G**
